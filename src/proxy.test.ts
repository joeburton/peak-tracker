...
type ProxyHandler = (auth: AuthFn, request: NextRequest) => Promise<Response | undefined>;
...
const req = (path: string) => new NextRequest(new URL(path, 'https://example.com'));

const authed = (): Promise<{ userId: string }> => Promise.resolve({ userId: 'user_test_123' });

const unauthed = (): Promise<{ userId: null }> => Promise.resolve({ userId: null });
