...
describe('PeakSchema', () => {
  ...
  it('accepts ISO 8601 strings with timezone offsets for createdAt and updatedAt', () => {
    expect(
      PeakSchema.safeParse({ ...validPeak, createdAt: '2024-01-01T00:00:00.000+01:00' }).success
    ).toBe(true);
    expect(
      PeakSchema.safeParse({ ...validPeak, updatedAt: '2024-06-15T12:30:00.000-05:00' }).success
    ).toBe(true);
  });
...
describe('UserProgressSchema', () => {
  ...
  it('accepts ISO 8601 strings with timezone offsets for updatedAt', () => {
    const result = UserProgressSchema.safeParse({
      ...validUserProgress,
      updatedAt: '2024-06-01T12:00:00.000+01:00',
    });
    expect(result.success).toBe(true);
  });
