...
describe('pullProgress', () => {
  ...
  it('does not update local when local is newer than server', async () => {
    ...
    const localRepo = makeLocalRepo({
      get: vi.fn().mockResolvedValue(newerLocal),
      upsert: mockUpsert,
    });
    const syncActions = makeSyncActions();
    const queryClient = makeQueryClient();
  ...
  it('uses version as tiebreaker when updatedAt values are equal', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          okResponse({ ...SERVER_PROGRESS, updatedAt: sameTimestamp, version: 10 })
        )
    );
    const localAtSameTime: LocalProgress = {
      ...LOCAL_PROGRESS,
    ...
    const mockUpsert = vi.fn();
    const localRepo = makeLocalRepo({
      get: vi.fn().mockResolvedValue(localAtSameTime),
      upsert: mockUpsert,
      markClean: vi.fn(),
    });
  ...
  it('does not update local when updatedAt and version are equal', async () => {
    ...
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(okResponse({ ...SERVER_PROGRESS, updatedAt: sameTimestamp, version: 5 }))
    );
    const localIdentical: LocalProgress = {
      ...LOCAL_PROGRESS,
      updatedAt: sameTimestamp,
      version: 5,
    };
    const mockUpsert = vi.fn();
    const localRepo = makeLocalRepo({
      get: vi.fn().mockResolvedValue(localIdentical),
      upsert: mockUpsert,
    });
  ...
  it('does not overwrite a dirty local record — push must complete first', async () => {
    ...
    const mockUpsert = vi.fn();
    const localRepo = makeLocalRepo({
      get: vi.fn().mockResolvedValue(dirtyLocal),
      upsert: mockUpsert,
    });
    const syncActions = makeSyncActions();
  ...
  it('calls setSyncError when the server response fails Zod validation', async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(okResponse({ userId: 'user_123', completedPeakIds: 'not-an-array' }))
    );
    const mockUpsert = vi.fn();
    const localRepo = makeLocalRepo({ upsert: mockUpsert });
    const syncActions = makeSyncActions();
    ...
    expect(mockUpsert).not.toHaveBeenCalled();
    expect(syncActions.setSyncError).toHaveBeenCalledWith(
      'Pull failed: invalid response from server'
    );
  });
});
